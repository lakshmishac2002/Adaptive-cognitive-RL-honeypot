#!/usr/bin/env python3
"""
Compact Reward Shaper for Maintaining Action Diversity
Drop this file into your project directory and import it.
"""
import numpy as np
...
class DiversityRewardShaper:
    ...
    def __init__(self, action_dim=6, diversity_bonus_weight=100.0):
        ...
        # Penalty multipliers for overused actions
        self.overuse_penalties = {'respond_realistic': 0.55,'delay_response' : 0.60,'fake_error': 0.65, 'fake_data': 0.60, 'block': 0.45}
    ...
    def shape_reward(self, base_reward, action, attack_type, step, done):
        ...
        Returns:
            Shaped reward (float)
        """
        base_reward = np.tanh(base_reward / 500.0) * 300 #Added
        ...
        if total > 100:
            ...
            # Penalty for overused actions
            if freq > expected * 1.3:
                action_name = self.action_names[action]
        ...
        # 4. Engagement bonus (reward keeping attacker engaged)
        reward += step * 2
        
        # 5. Penalty for blocking too early
        if done and action < len(self.action_names):
            if self.action_names[action] == 'block' and step < 20:
                reward -= 200
...
if __name__ == "__main__":
    ...
    # Test scenario: overusing respond_realistic
    print("\nScenario 2: Overusing respond_realistic (like your current agent)")
    shaper.action_counts = np.array([673, 38, 29, 203, 29, 27])
    ...
    print(f"  Shaped (respond_realistic): {shaped_over:.2f} [penalty applied]")
    print(f"  Shaped (fake_error, good for SQL): {shaped_under:.2f} [bonus applied]")
    stats = shaper.get_statistics()
    print(f"  Diversity: {stats['diversity_percent']:.1f}%")
    
    print("\n✓ Reward shaper ready to use!")
